...
- [ ] **Unicode invoice PDF** — `invoice_pdf.py` still downgrades non-Latin-1 names to `?` (the `def0c1c` fix). Bundle an OFL Noto Sans (Latin + Devanagari + Oriya) TTF with `add_font`/`set_fallback_fonts` to render real regional names; keep `_latin1()` as a graceful fallback. **(Phase 5 of the multilingual plan — not yet started.)**
