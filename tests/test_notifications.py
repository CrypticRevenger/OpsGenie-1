...
async def _make_company(db: AsyncSession) -> Company:
    company = Company(
        business_name="Notify Test Co", owner_name="Owner", whatsapp_number=_unique_phone()
    )
    db.add(company)
    await db.commit()
...
@pytest.mark.asyncio
async def test_stale_data_alert_fires_to_founder(db: AsyncSession, recorded_sends, monkeypatch):
    founder = _unique_phone()
    _set_founder_number(monkeypatch, founder)
    company = await _make_company(db)
    snap = _snapshot(company.id, data_freshness_hours=30.0)

    fired = await check_stale_data_alert(db, company, snap, NOW)
    await db.commit()
...
@pytest.mark.asyncio
async def test_stale_data_alert_not_fired_when_fresh(db: AsyncSession, recorded_sends, monkeypatch):
    _set_founder_number(monkeypatch, _unique_phone())
    company = await _make_company(db)
    snap = _snapshot(company.id, data_freshness_hours=2.0)
    fired = await check_stale_data_alert(db, company, snap, NOW)
    await db.commit()
...
@pytest.mark.asyncio
async def test_stale_data_alert_dedups_same_day(db: AsyncSession, recorded_sends, monkeypatch):
    _set_founder_number(monkeypatch, _unique_phone())
    company = await _make_company(db)
    snap = _snapshot(company.id, data_freshness_hours=None)

    first = await check_stale_data_alert(db, company, snap, NOW)
    await db.commit()
    second = await check_stale_data_alert(db, company, snap, NOW)
    await db.commit()
